...
namespace DodoTheGame.ForegroundEffects
{
  internal class HarvestResults : IForegroundEffect
  {
    ...
    public void Draw(SpriteBatch sb, GameTime gameTime, Vector2 backgroundPos)
    {
      ...
      float num2 = 1f;
      float num3 = (float) ((double) this.objectEpicenter.Y - 75.0 * (double) Convert.ToSingle(1.0 - (num1 - gameTime.TotalGameTime.TotalMilliseconds) / 1000.0) - 100.0);
      ...
      foreach (ItemStack harvestResult in this.harvestResults)
      {
        ...
        {
          Recorder.RDrawString(sb, this.spriteFont, harvestResult.count.ToString() + "x", new Vector2(vector2.X, vector2.Y + (float) (num4 * 50)), Color.Black * num2);
          Recorder.RDraw(sb, Game1.smallItemTextures[harvestResult.itemId], new Vector2((float) ((double) vector2.X + (double) x + 0.0), vector2.Y + (float) (num4 * 50)), Color.White * num2);
        }
        else
          Recorder.RDraw(sb, Game1.staticMiniFlowersTextures[harvestResult.itemId - 81], new Vector2((float) ((double) vector2.X + (double) x + 0.0), vector2.Y + (float) (num4 * 50)), Color.White * num2);
        ++num4;
      }
    }
  }
