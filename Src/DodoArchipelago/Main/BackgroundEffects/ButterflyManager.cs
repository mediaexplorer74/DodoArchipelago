...
namespace DodoTheGame.BackgroundEffects
{
  internal static class ButterflyManager
  {
    ...
    {
      if (Game1.rand.Next(0, 100) > 90 && ButterflyManager.butterflyList.Count < 11)
      {
        Vector2 startingPoint;
        Vector2 generalGuidance;
        switch (Game1.rand.Next(0, 4))
        {
          case 0:
            startingPoint = player.location + new Vector2((float) Game1.rand.Next(-600, 1880), -700f);
            generalGuidance = new Vector2(0.0f, -2f);
            break;
          case 1:
            startingPoint = player.location + new Vector2(2000f, (float) Game1.rand.Next(-600, 1320));
            generalGuidance = new Vector2(-2f, 0.0f);
            break;
          case 2:
            startingPoint = player.location + new Vector2((float) Game1.rand.Next(-600, 1880), 1320f);
            generalGuidance = new Vector2(0.0f, 2f);
            break;
          default:
            startingPoint = player.location + new Vector2(-600f, (float) Game1.rand.Next(-600, 1320));
            generalGuidance = new Vector2(2f, 0.0f);
            break;
        }
