...
namespace DodoTheGame.GUI
{
  internal static class GUIManager
  {
    ...
    public static TutorialGUI tuto;//tutorialHUD;
    ...
    public static void Setup()
    {
      ...
      GUIManager.tuto = new TutorialGUI();//CreditsGUI();
        }
    ...
    public static void LoadAssets(Game1 game)
    {
      ...
      GUIManager.settings.retour_selected = ContentLoadingWrapper.Load<Texture2D>(
          "mainmenu/parameters/retour_selected");
      ...
      GUIManager.settings.more_mirror_s = ContentLoadingWrapper.Load<Texture2D>(
          "mainmenu/parameters/more_mirror_white");
      GUIManager.settings.mainbackground = ContentLoadingWrapper.Load<Texture2D>(
          "mainmenu/bg");
      ...
      GUIManager.editorHUD.background = ContentLoadingWrapper.Load<Texture2D>("ui/pause1");
        ...
        // RnD
        //GUIManager.tuto.mainbackground = ContentLoadingWrapper.Load<Texture2D>("mainmenu/bg");
        //GUIManager.tuto.tutorialbackground = ContentLoadingWrapper.Load<Texture2D>("mainmenu/credits");

        GUIManager.tuto.animatedBackground =
         new Sprite("tuto", ContentLoadingWrapper.Load<Texture2D>(/*"ui/tuto"*/"ui/inv_bg_animated"))
      {
        animated = true,
        Width = 751,
        MillisecondsPerFrame = 12,
    ...
    public static void Update(GameTime gameTime)
    {
      ...
      (
        ...
        || GUIManager.currentHUDs.Contains((IGUI) GUIManager.tuto)
      )
    ...
    public static void GUIInput(UserInputStatus uis, Game1 game)
    {
      ...
      if ((double) uis.moveDown > 0.4 && (double) GUIManager.lastUIS.moveDown <= 0.4)
       ...
       //TODO: fix "Collection was modified; enumeration operation may not execute." bug
       try
       {
        foreach (IGUI currentHuD in GUIManager.currentHUDs)
        {
            ...
            down, 
            action1, 
            uis.moveDown > 0.4  || uis.spacebar == UserInputStatus.InputState.Held, 
            uis.alt, 
            game);
        }
      }
