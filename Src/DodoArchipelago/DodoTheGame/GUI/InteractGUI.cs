    ...
    public void Draw(SpriteBatch spriteBatch, Game1 game, GameTime gameTime)
    {
      ...
      if (flag || (double) this.compassOpacity > 0.0)
      {
        ...
        if (num >= -135.0 && num <= -45.0)
        {
          ...
          this.dir = (double) single <= 820.0 ? ((double) single >= 460.0 ? this.dirdown : this.dirleftdown) : this.dirdownright;
          float y = (double) single <= 880.0 ? ((double) single >= 350.0 ? 640f : 555f) : 515f;
        ...
        else if (num >= 45.0 && num < 135.0)
        {
          ...
          this.dir = (double) single <= 820.0 ? ((double) single >= 460.0 ? this.dirup : this.dirupleft) : this.dirupright;
        ...
        else if (num > -45.0 && num < 45.0)
        {
          ...
          this.dir = (double) single <= 480.0 ? ((double) single >= 240.0 ? this.dirleft : this.dirupleft) : this.dirleftdown;
          location = new Vector2(10f, (double) single < 555.0 ? single : 555f);
        }
        else if (num >= 135.0 && num <= 180.0 || num >= -180.0 && num < -135.0)
        {
          float single = Convert.ToSingle(Math.Round(10.0 + 630.0 * (num < 135.0 || num > 180.0 ? (num + 180.0) / 90.0 + 0.5 : (num - 135.0) / 90.0)));
          this.dir = (double) single <= 480.0 ? ((double) single >= 240.0 ? this.dirright : this.dirupright) : this.dirdownright;
      ...
      if (game.userInput.inputMethod == 1 || game.userInput.inputMethod == 3)
      {
        ...
        if (Game1.player.unlockedPlayerTools[PlayerUnlockables.PlayerUnlockable.Bike])
        {
          ...
          Recorder.RDraw(spriteBatch, this.bike_icon, new Vector2(180f, renderSize.Y - 70f), Color.White * num9);
        }
        if (Game1.player.unlockedPlayerTools[PlayerUnlockables.PlayerUnlockable.Bicycle])
        {
          ...
          Recorder.RDraw(spriteBatch, this.bicycle_icon, new Vector2(295f, renderSize.Y - 70f), Color.White * num10);
        }
      }
