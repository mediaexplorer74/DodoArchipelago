...
namespace DodoTheGame.BackgroundEffects
{
  internal static class WindTrailManager
  {
    ...
    public static void Update(Player player, GameTime gameTime)
    {
      ...
      if (Game1.rand.Next(0, 100) <= (player.currentMovementType == Player.DodoMovement.Bike ? 50 : 100 - (int) Convert.ToInt16(Math.Round(DayCycle.timeSpeed, 0)) * 10) || WindTrailManager.trailList.Count >= (player.currentMovementType == Player.DodoMovement.Bike ? 10 : (int) Convert.ToInt16(Math.Round(DayCycle.timeSpeed, 0)) * 100 - 5))
        return;
        ...
        if (getAngle != Math.PI / 4.0)
        {
          if (getAngle != 3.0 * Math.PI / 4.0)
          {
            if (getAngle != 5.0 * Math.PI / 4.0)
            {
              ...
              {
                loc = new Vector2(player.location.X - (float) Game1.rand.Next(-600, 900), player.location.Y + (float) Game1.rand.Next(-500, 1000));
                direction = WindTrail.WindDirection.UpLeft;
              }
              else
              {
                loc = new Vector2(player.location.X + (float) Game1.rand.Next(-500, 1000), player.location.Y + (float) Game1.rand.Next(-750, 750));
                direction = WindTrail.WindDirection.Left;
              }
            }
            else
            {
              loc = new Vector2(player.location.X + (float) Game1.rand.Next(-600, 900), player.location.Y - (float) Game1.rand.Next(-1000, 500));
              direction = WindTrail.WindDirection.DownLeft;
            }
          }
          else
          {
            loc = new Vector2(player.location.X + (float) Game1.rand.Next(-900, 600), player.location.Y - (float) Game1.rand.Next(-1000, 500));
            direction = WindTrail.WindDirection.DownRight;
          }
        }
        else
        {
          loc = new Vector2(player.location.X - (float) Game1.rand.Next(-900, 600), player.location.Y + (float) Game1.rand.Next(-500, 1000));
          direction = WindTrail.WindDirection.UpRight;
        }
      }
      else
      {
        loc = new Vector2(player.location.X + (float) Game1.rand.Next(-1000, 500), player.location.Y + (float) Game1.rand.Next(-750, 750));
      ...
      if (WindTrailManager.trailList.Count<WindTrail>((Func<WindTrail, bool>) (p => (double) Math.Abs(p.startingPoint.Y - loc.Y) < 100.0)) != 0)
        return;
      WindTrailManager.trailList.Add(new WindTrail(loc, Game1.windSprites[Game1.rand.Next(0, Game1.windSprites.Length)], direction));
    }
