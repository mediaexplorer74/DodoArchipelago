...
{
  /*internal*/public static class WorldGenerator
  {

    //public static void NewWorldGen(World @world, List<Preset> presetList)
    public static List<IWorldObject> ManualFormWorldObjects(World @world, List<Preset> presetList)
    {
        System.Diagnostics.Debug.WriteLine("[i] NewWorldGen - Generating buildpoints - start...");
            ...
            BuildPoint bp1 = new BuildPoint
            (
                "bp1", 
                Game1.buildPointSprite, 
                new Vector2(8985, 7932), 
                null, 
                new Vector2(8985 + 20, 7932 + 65))
            {
                ExtraReach = new Vector2 { Y = -8 },
                ExtraFloorHeight = 20,
                Interactions = 
                { 
                   [(int)Cardinal.Up] = new Build(
                   ...
                   presetList.First(p => p.name == "Tableau de bord"))
                },
                Tags = new string[] { "boardAnimation" },
                requiredLevel = 0/*((BuildPoint)worldObject).requiredLevel.ToString()*/,
                bpIncompatibleTags = new string[] { "" },
                isBpVisible = true,

            };
            
                  
            world.objects.Add(bp1);


            /*
              Result of build :
          world.objects.AddRange( 
           presetList.First(p => p.name == "Tableau de bord")
         .MakeWOs(new List<Tuple<Vector2, string>>()
         {
              Tuple.Create(
                  new Vector2(8985-160, 7932-160),
                      "Tableau de bord")
         }));
          */
        ...
        //Vector2 location = new Vector2(8700, 9000);
            ...
            .MakeWOs(new List<Tuple<Vector2, string>>()
        {
            Tuple.Create(new Vector2(
                   Game1.dodoCenteredScreenLocation.X-50,
                    Game1.dodoCenteredScreenLocation.X-50),
            "UpgradableHouse2")
        }));
