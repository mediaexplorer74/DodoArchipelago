...
namespace DodoTheGame.GUI
{
  internal class LoadGUI : IGUI
  {
    ...
    {
      this.img1 = this.saveInfo1 == null ? this.empty : (this.saveInfo1.isValid ? LoadGUI.GenerateThumbnail(this.saveInfo1.screenshot, graphics) : this.error);
      this.img2 = this.saveInfo2 == null ? this.empty : (this.saveInfo2.isValid ? LoadGUI.GenerateThumbnail(this.saveInfo2.screenshot, graphics) : this.error);
      if (this.saveInfo3 != null)
        this.img3 = this.saveInfo3.isValid ? LoadGUI.GenerateThumbnail(this.saveInfo3.screenshot, graphics) : this.error;
      else
    ...
    private static Texture2D GenerateThumbnail(Texture2D input, GraphicsDevice graphics)
    {
      ...
      Rectangle rectangle = new Rectangle(340, 199, texture2D.Width - 680, texture2D.Height - 398);
      Texture2D thumbnail = new Texture2D(graphics, rectangle.Width, rectangle.Height);
      Color[] data = new Color[rectangle.Width * rectangle.Height];
      texture2D.GetData<Color>(0, new Rectangle?(rectangle), data, 0, data.Length);
      ...
      Game1 game)
    {
      ...
      if (!flag)
        return;
      CutsceneManager.StartCutscene((ICutscene) new StoryIntroCutscene(), (GameTime) null, Game1.player, Game1.world);
    }
    ...
    public void Draw(SpriteBatch spriteBatch, Game1 game, GameTime gameTime)
    {
      ...
      int int32 = Convert.ToInt32(Math.Round((double) Game1.renderSize.X / 2.0 - (double) (texture1.Width / 2), 0));
      ...
      Recorder.RDraw(spriteBatch, this.img1, new Rectangle(int32 + 3, 155, 234, 125), Color.White);
      Recorder.RDraw(spriteBatch, this.img2, new Rectangle(int32 + 3, 315, 234, 125), Color.White);
      Recorder.RDraw(spriteBatch, this.img3, new Rectangle(int32 + 3, 475, 234, 125), Color.White);
      ...
      Recorder.RDraw(spriteBatch, texture4, new Vector2((float) (int32 + texture1.Width + 10), 240f), Color.White);
      Recorder.RDraw(spriteBatch, texture5, new Vector2((float) (int32 + texture1.Width + 10), 400f), Color.White);
      Recorder.RDraw(spriteBatch, texture6, new Vector2((float) (int32 + texture1.Width + 10), 560f), Color.White);
      Recorder.RDraw(spriteBatch, this.marker1, new Vector2((float) (int32 + 240), 158f), Color.White);
      if (this.saveInfo1.isValid)
      {
        Recorder.RDrawString(spriteBatch, this.font16, LoadGUI.FormatPlaytime(this.saveInfo1.playTime), new Vector2((float) (int32 + 246), 229f), Color.White);
        Recorder.RDrawString(spriteBatch, this.font22, LoadGUI.FormatDatetime(this.saveInfo1.saveTime), new Vector2((float) (int32 + 246), 249f), Color.White);
      }
      Recorder.RDraw(spriteBatch, this.marker2, new Vector2((float) (int32 + 240), 318f), Color.White);
      if (this.saveInfo2.isValid)
      {
        Recorder.RDrawString(spriteBatch, this.font16, LoadGUI.FormatPlaytime(this.saveInfo2.playTime), new Vector2((float) (int32 + 246), 389f), Color.White);
        Recorder.RDrawString(spriteBatch, this.font22, LoadGUI.FormatDatetime(this.saveInfo2.saveTime), new Vector2((float) (int32 + 246), 409f), Color.White);
      }
      Recorder.RDraw(spriteBatch, this.marker3, new Vector2((float) (int32 + 240), 478f), Color.White);
      if (!this.saveInfo3.isValid)
        return;
      Recorder.RDrawString(spriteBatch, this.font16, LoadGUI.FormatPlaytime(this.saveInfo3.playTime), new Vector2((float) (int32 + 246), 549f), Color.White);
      Recorder.RDrawString(spriteBatch, this.font22, LoadGUI.FormatDatetime(this.saveInfo3.saveTime), new Vector2((float) (int32 + 246), 569f), Color.White);
    }

    private static string FormatDatetime(DateTime dt)
    {
      return dt.Day.ToString().PadLeft(2, '0') + "/" + dt.Month.ToString().PadLeft(2, '0') + "/" + dt.Year.ToString().Substring(2, 2) + " " + dt.Hour.ToString().PadLeft(2, '0') + "h" + dt.Minute.ToString().PadLeft(2, '0');
    }

    private static string FormatPlaytime(int pt)
    {
      TimeSpan timeSpan = TimeSpan.FromSeconds((double) pt);
      return timeSpan.TotalMinutes < 60.0 ? string.Format(LocalizationManager.GetString("PlaytimeMinutes"), (object) timeSpan.Minutes) : string.Format(LocalizationManager.GetString("PlaytimeMinutesHours"), (object) Math.Floor(timeSpan.TotalHours), (object) timeSpan.Minutes);
    }
