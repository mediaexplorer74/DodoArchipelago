  ...
  internal class LoadGUI : IGUI
  {
    ...
    public void GenerateThumbnails(GraphicsDevice graphics)
    {
      this.img1 = this.saveInfo1 == null 
                ? this.empty : (this.saveInfo1.isValid
      ...
      Game1 game)
    {
      ...
      if (left && this.selectedButton != 1 && this.selectedButton != 2
                && this.selectedButton != 3)
        this.selectedButton -= 3;

      if (right && this.selectedButton != 4 && this.selectedButton != 5 
    ...
    public void Draw(SpriteBatch spriteBatch, Game1 game, GameTime gameTime)
    {
      ...
      int int32 = Convert.ToInt32(Math.Round((double) Game1.renderSize.X / 2.0 
          - (double) (texture1.Width / 2), 0));
      ...
      {
        Recorder.RDrawString(spriteBatch, this.font16, 
            LoadGUI.FormatPlaytime(this.saveInfo1.playTime), 
            new Vector2((float) (int32 + 246), 229f), Color.White);
        Recorder.RDrawString(spriteBatch, this.font22, 
            LoadGUI.FormatDatetime(this.saveInfo1.saveTime), 
      ...
      {
        Recorder.RDrawString(spriteBatch, this.font16, 
            LoadGUI.FormatPlaytime(this.saveInfo2.playTime), new Vector2((float) (int32 + 246), 389f), 
            Color.White);
        Recorder.RDrawString(spriteBatch, this.font22, 
            LoadGUI.FormatDatetime(this.saveInfo2.saveTime), new Vector2((float) (int32 + 246), 409f), 
            Color.White);
      }
    ...
    private static string FormatDatetime(DateTime dt)
    {
      return dt.Day.ToString().PadLeft(2, '0') + "/" + dt.Month.ToString().PadLeft(2, '0') 
                + "/" + dt.Year.ToString().Substring(2, 2) + " " + dt.Hour.ToString().PadLeft(2, '0') + "h" + dt.Minute.ToString().PadLeft(2, '0');
    }
    ...
    private static string FormatPlaytime(int pt)
    {
      ...
      return timeSpan.TotalMinutes < 60.0 
                ...
                : string.Format(LocalizationManager.GetString("PlaytimeMinutesHours"),
                         (object) Math.Floor(timeSpan.TotalHours), 
                 (object) timeSpan.Minutes);
    }
