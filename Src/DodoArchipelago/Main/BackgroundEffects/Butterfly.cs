...
namespace DodoTheGame.BackgroundEffects
{
  internal class Butterfly : IBackgroundEffect
  {
    ...
    private IEnumerable<Vector2> GeneratePoints(int count, Vector2 initialLocation)
    {
      ...
      {
        source.Add(vector2);
        vector2.X += (float) Game1.rand.Next(-this.amplitude + Convert.ToInt32(this.generalGuidance.X * 0.1f), this.amplitude + Convert.ToInt32(this.generalGuidance.X));
        vector2.Y += (float) Game1.rand.Next(-this.amplitude + Convert.ToInt32(this.generalGuidance.Y * 0.1f), this.amplitude + Convert.ToInt32(this.generalGuidance.Y));
      }
      float[] xs;
      float[] ys;
